...
logs.usage = [
  ...
  'Example usages:',
  'jitsu logs all',
  'jitsu logs all <number of lines to show>|stream',
  'jitsu logs app <app name>',
  'jitsu logs app <app name> <number of lines to show>|stream'
];
...
function putStream(stream, callback) {
  ...
  stream.on('data', function(data) {
    ...
    for (; i < l; i++) {
      output(log[i], j);
      buff = buff.substring(log[i].length);
    }
  });
